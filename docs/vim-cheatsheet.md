...
| `set clipboard=unnamedplus` | `y`/`p` use the system clipboard directly | Vim's own clipboard, separate from system |
| `set undofile` + `undodir=~/.vim/undodir//` | Undo history persists across sessions, stored in `~/.vim/undodir/`; the `//` suffix means full file paths are encoded in undo filenames so two `foo.py` in different dirs don't collide | No persistent undo |
...
| `gd / gr / K / [d / ]d` | LSP navigation via coc.nvim | No LSP; `gd` goes to local definition only |
| `Tab` in insert mode | Triggers coc.nvim completion menu | Inserts a literal tab (or spaces per expandtab) |
...
| `:Files` | Fuzzy file finder (fzf.vim) | ivy/helm `C-x C-f` |
...
| `:Buffers` | Fuzzy buffer switcher (fzf.vim) | |
...
With `clipboard=unnamedplus` in your vimrc, plain `y`/`p` already use the system clipboard.
